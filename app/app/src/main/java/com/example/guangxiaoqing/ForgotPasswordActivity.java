...
public class ForgotPasswordActivity extends AppCompatActivity {
    ...
    private Button btnGetVerificationCode, btnResetPassword;
        ...
        etVerificationCode.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                if (hasFocus && !isVerificationCodeSent && !TextUtils.isEmpty(etVerificationCode.getText())) {
                    ToastHelper.showInfo(ForgotPasswordActivity.this, "请先获取验证码");
        ...
        btnGetVerificationCode.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (validatePhone()) {
                    ...
                    android.util.Log.d("ForgotPasswordActivity", "发送验证码请求: 手机号=" + phone);

                    // 创建NetworkService实例
                    com.example.guangxiaoqing.api.NetworkService networkService =
                            new com.example.guangxiaoqing.api.NetworkService(ForgotPasswordActivity.this);

                    // 调用sendSms方法
                    networkService.sendSms(phone, "password_reset", new com.example.guangxiaoqing.api.NetworkCallback<Object>() {
                        @Override
                        public void onSuccess(Object response) {
                            android.util.Log.d("ForgotPasswordActivity", "验证码请求成功: " + response);
                            startCountDownTimer();
                            // 设置验证码已发送标志和已验证的手机号
                            isVerificationCodeSent = true;
                            verifiedPhone = etPhone.getText().toString().trim();
                            ToastHelper.showSuccess(ForgotPasswordActivity.this, "验证码已发送");
                        }

                        @Override
                        public void onError(String errorMessage) {
                            android.util.Log.e("ForgotPasswordActivity", "验证码请求失败: " + errorMessage);
                            // 重置验证码发送状态
                            isVerificationCodeSent = false;
                            verifiedPhone = "";
                            ToastHelper.showError(ForgotPasswordActivity.this, errorMessage);
                        }
                    });
                }
            }
        ...
        btnResetPassword.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (validateAllInputs()) {
                    ...
                    // 使用NetworkService进行密码重置
                    android.util.Log.d("ForgotPasswordActivity", "发送密码重置请求: 手机号=" + phone);

                    // 创建NetworkService实例
                    com.example.guangxiaoqing.api.NetworkService networkService =
                            new com.example.guangxiaoqing.api.NetworkService(ForgotPasswordActivity.this);

                    // 直接调用resetPassword方法（不需要token的密码重置）
                    // 后端会在密码重置过程中验证验证码
                    android.util.Log.d("ForgotPasswordActivity", "开始密码重置流程");
                    networkService.resetPassword(phone, verificationCode, newPassword, new com.example.guangxiaoqing.api.NetworkCallback<com.example.guangxiaoqing.model.TokenResponse>() {
                        @Override
                        public void onSuccess(com.example.guangxiaoqing.model.TokenResponse response) {
                            android.util.Log.d("ForgotPasswordActivity", "密码重置成功: token=" + response.getAccessToken());
                            ToastHelper.showSuccess(ForgotPasswordActivity.this, "密码重置成功");
                        ...
                        @Override
                        public void onError(String errorMessage) {
                            android.util.Log.e("ForgotPasswordActivity", "密码重置失败: " + errorMessage);
                            ToastHelper.showError(ForgotPasswordActivity.this, errorMessage);
                        }
                    });
                }
    ...
    private boolean validateAllInputs() {
        ...
        if (TextUtils.isEmpty(phone) && TextUtils.isEmpty(verificationCode) &&
            ...
            ToastHelper.showInfo(this, "请填写密码重置信息");
            return false;
        }
        ...
        if (TextUtils.isEmpty(password)) {
            ToastHelper.showInfo(this, "请重置密码");
            isValid = false;
        ...
        countDownTimer = new CountDownTimer(VERIFICATION_CODE_TIMEOUT, 1000) {
            ...
            public void onFinish() {
                ...
                if (isVerificationCodeSent) {
                    isVerificationCodeSent = false;
                    verifiedPhone = "";
                    ToastHelper.showInfo(ForgotPasswordActivity.this, "验证码已过期，请重新获取");
                }
            }
        }.start();
    }
